...
,
        SUM(urineoutput) OVER (
            ...
            ORDER BY seconds_since_admit RANGE BETWEEN 21600 PRECEDING AND CURRENT ROW
        ) AS urineoutput_6hr,
        SUM(urineoutput) OVER (
            PARTITION BY stay_id
            ORDER BY seconds_since_admit RANGE BETWEEN 43200 PRECEDING AND CURRENT ROW
        ) AS urineoutput_12hr,
        SUM(urineoutput) OVER (
            PARTITION BY stay_id
            ORDER BY seconds_since_admit RANGE BETWEEN 86400 PRECEDING AND CURRENT ROW
        ) AS urineoutput_24hr -- repeat the summations using the hours_since_previous_row column
...
SELECT ur.stay_id,
    ...
    ur.urineoutput_6hr,
    ur.urineoutput_12hr,
    ur.urineoutput_24hr -- calculate rates while requiring UO documentation over at least N hours
...
,
    CASE
        ...
        AND uo_tm_6hr < 12 THEN ROUND(
            CAST(
                (ur.urineoutput_6hr / wd.weight / uo_tm_6hr) AS NUMERIC
            ),
            4
        )
        ELSE NULL
    ...
    CASE
        WHEN uo_tm_12hr >= 12 THEN ROUND(
            CAST(
                (ur.urineoutput_12hr / wd.weight / uo_tm_12hr) AS NUMERIC
            ),
            4
        )
        ELSE NULL
    ...
    CASE
        WHEN uo_tm_24hr >= 24 THEN ROUND(
            CAST(
                (ur.urineoutput_24hr / wd.weight / uo_tm_24hr) AS NUMERIC
            ),
            4
        )
        ELSE NULL
    END AS uo_rt_24hr -- number of hours between current UO time and earliest charted UO
