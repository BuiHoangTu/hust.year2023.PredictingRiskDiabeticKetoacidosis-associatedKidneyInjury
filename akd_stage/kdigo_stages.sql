...
WITH cr_stg AS (
    SELECT cr.stay_id,
        ...
        CASE
            -- 3x baseline
            WHEN cr.creat >= (cr.creat_low_past_7day * 3.0) THEN 3 -- *OR* cr >= 4.0 with associated increase
            WHEN cr.creat >= 4 -- For patients reaching Stage 3 by SCr >4.0 mg/dl
...
,
        CASE
            ...
            AND uo.uo_rt_24hr < 0.3 THEN 3 -- *or* anuria for >= 12 hours
            WHEN uo.uo_tm_12hr >= 12
            AND uo.uo_rt_12hr = 0 THEN 3 -- Stage 2: <0.5 ml/kg/h for >= 12 hours
            WHEN uo.uo_tm_12hr >= 12
            AND uo.uo_rt_12hr < 0.5 THEN 2 -- Stage 1: <0.5 ml/kg/h for 6–12 hours
            WHEN uo.uo_tm_6hr >= 6
...
,
    ...
    ) AS aki_stage -- We intend to combine together the scores from creatinine/UO by left
...
FROM icustays ie -- get all possible charttimes as listed in tm_stg
    ...
    LEFT JOIN crrt_stg crrt ON ie.stay_id = crrt.stay_id
